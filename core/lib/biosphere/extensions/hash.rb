module Biosphere
  module Extensions
    module HashExtensions
      ...
      def flatten_keys
        result = {}
        flatten_keys_of(self, false) do |key, value|
      ...
      def flatten_keys_of(hash, escape, prev_key=nil, &block)
      ...
      #def flatten_keys(hash=self, escape=false, prev_key=nil, &block)
      #  hash.each_pair do |key, value|
      #    key = escape_default_separator(key) if escape
      #    curr_key = [prev_key, key].compact.join('.').to_sym
      #    yield curr_key, value
    ...
    module Flatten
      ...
      def self.normalize_flat_keys(locale, key, scope, separator)
        ...
        if separator != FLATTEN_SEPARATOR
          keys.map! do |k|
            k.to_s.tr("#{FLATTEN_SEPARATOR}#{separator}", "#{SEPARATOR_ESCAPE_CHAR}#{FLATTEN_SEPARATOR}")
          end
        end
